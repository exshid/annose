type ServiceProps = {
    photo: string;
  };
  
  export function Service() {
    
    return (
        <div className="bg-darkerlight dark:bg-darkish text-darkish dark:text-white flex justify-center h-screen w-full">
        <div className="w-8/12 flex flex-col">
     
    <h3 className="font-inter text-5xl font-black uppercase bg-gradient-to-r from-cyan-400 to-cyan-500 bg-clip-text text-transparent">
        Service for Authors</h3>
<ul className="flex flex-wrap">
    <li className="w-1/3 h-64 text-white my-5">
        <div className="bg-gradient-to-r h-full from-cyan-400 to-cyan-500 m-5">
<h4>Novel editing</h4>
<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Illo tempore inventore fugiat mlaceat nom odit delectus maxime nam temporibus distinctio.</p>
    </div>
    </li>

    <li className="w-1/3 h-64 text-white my-5">
        <div className="bg-gradient-to-r h-full from-cyan-400 to-cyan-500 m-5">
<h4>Novel editing</h4>
<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Illo tempore inventore fugiat mlaceat nom odit delectus maxime nam temporibus distinctio.</p>
    </div>
    </li>
    <li className="w-1/3 h-64 text-white my-5">
        <div className="bg-gradient-to-r h-full from-cyan-400 to-cyan-500 m-5">
<h4>Novel editing</h4>
<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Illo tempore inventore fugiat mlaceat nom odit delectus maxime nam temporibus distinctio.</p>
    </div>
    </li>
    <li className="w-1/3 h-64 text-white my-5">
        <div className="bg-gradient-to-r h-full from-cyan-400 to-cyan-500 m-5">
<h4>Novel editing</h4>
<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Illo tempore inventore fugiat mlaceat nom odit delectus maxime nam temporibus distinctio.</p>
    </div>
    </li>

    <li className="w-1/3 h-64 text-white my-5">
        <div className="bg-gradient-to-r h-full from-cyan-400 to-cyan-500 m-5">
<h4>Novel editing</h4>
<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Illo tempore inventore fugiat mlaceat nom odit delectus maxime nam temporibus distinctio.</p>
    </div>
    </li>
    <li className="w-1/3 h-64 text-white my-5">
        <div className="bg-gradient-to-r h-full from-cyan-400 to-cyan-500 m-5">
<h4>Novel editing</h4>
<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Illo tempore inventore fugiat mlaceat nom odit delectus maxime nam temporibus distinctio.</p>
    </div>
    </li>

</ul>
 </div>
 </div>
 
);
  }
