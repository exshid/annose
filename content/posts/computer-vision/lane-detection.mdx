...
First, we need a dashcam video to orkwith. I chosse [this one](https://youtu.be/zSY1Ux7UbPU?t=3638) because it features multiple lane changes, both left and right. A frame from the video looks like this:

![Dashscam video frame {{ w: 700, h: 393, cap: "The first frame of the video" }}](/images/computer-vision/lane-detection/first-frame.webp)

Let's import the necessfary libraries, `cv2{:python}` for OpenCV and `numpy{:python}` for working with arrays:
